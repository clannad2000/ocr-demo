...
`ocr_demo.py --export-pdf chinese`通过`pdf_backfill.py`复用本工具的字形掩码和
OpenCV修复核心，但两种入口的删除范围不同：

- 独立CLI默认处理`layout.content`中除底部页码外的全部定位区域。
- PDF回填只处理已经通过匹配、置信度和覆盖规则，并且最终确实要写入中文的
  `erase_boxes`；跳过、未匹配和低置信度区域保持原样。
- PDF回填把修复像素作为透明图层覆盖回原PDF，其余页面内容仍使用原PDF底图，
  中文继续作为矢量文字写入。
- 局部版面覆盖显式提供`background_color`时，该区域仍可使用纯色矩形作为人工
  修正；这不是默认擦除策略。
...
`--pages-dir`会按文件名排序，处理目录内每个`.png`以及其同名`.json`，例如
`page-0001.png`与`page-0001.json`。任一PNG缺少同名JSON时会在写入前报错。它不能与
`--image`或`--json`同时使用。

```powershell
python .\erase_english_from_deepseek.py `
  --pages-dir .\runs\study-batch-3A\pages `
  --output-dir .\runs\book-review-3A\erase
```
...
| 参数 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `--image` | 与`--pages-dir`二选一 | 无 | 原始图片路径；必须同时提供`--json`。 |
| `--pages-dir` | 与`--image`二选一 | 无 | 包含同名`.png`和`.json`页面对的目录。 |
| `--json` | 单页模式必填 | 无 | 现有 DeepSeek-OCR JSON 路径；不能与`--pages-dir`同用。 |
