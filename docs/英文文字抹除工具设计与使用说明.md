...
- 不负责将中文翻译重新排版到气泡中。
- 目录批处理要求每张PNG存在同名JSON；缺失时在写入前停止。
- 不保证区域类别在所有漫画版式下都能准确识别；区域类别当前主要用于调试和元数据记录。

### 2.3 与PDF回写的边界

本工具是五步流程中唯一负责字形掩码和OpenCV背景修复的入口。它生成
`page-NNNN.cleaned.png`；`pdf_translation_writer.py`只读取cleaned PNG、锁定译文
快照和坐标计划并写入中文，不执行或复用擦除逻辑。
