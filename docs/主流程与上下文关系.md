...
### 阶段 9：中文 PDF 回填与程序全量检查

**做什么**

- 读取锁定译文快照；
- 将最终译文区域映射到版面 OCR 坐标；
- 在原 PDF 页面上局部修复英文并叠加矢量中文；
...
`--export-pdf chinese` 会自动重建最终译文快照，因此阶段 8 的命令可以省略；单独执行阶段 8
更适合先审计快照内容。
...
| 中文回填 PDF | 源 PDF + 锁定快照 + 坐标和回填规则 | 不再判断译文 | 最终视觉验收对象 |
